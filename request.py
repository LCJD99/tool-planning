...
class ApiClient:
    ...
    def send_request(self, prompt: str, max_iterations: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Request completed response: {response}, {elapsed:.2f}s")
    ...
    def run_simulation(self, prompts: List[str], max_iterations: int = 5, num_requests = 1) -> None:
        """
        ...
        Args:
            ...
            max_iterations: Maximum number of iterations for each request
        """
        asyncio.run(self.run_simulation_async(prompts, max_iterations, num_requests))
...
async def async_main():
    ...
    parser.add_argument("--rate", type=float, default=2, help="Average request rate per minute (lambda)")
    ...
    parser.add_argument("--num-requests", type=int, default=3, help="Maximum iterations for each request")
    ...
    await client.run_simulation_async(prompts, args.max_iterations, args.num_requests)
